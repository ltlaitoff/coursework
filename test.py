...
begin
  ...
  text := 'TRANSFORM Max(m.mark) AS [Max-mark_id] SELECT t.fullname FROM (SELECT testing1.fullname, IIf(testing1.date = testing3.firs, testing1.mark_id, Null) AS mark_id, testing3.firs FROM ( SELECT (u.surname & " " & u.name) AS fullname, j.mark_id, j.date FROM  ( SELECT * FROM Journal WHERE ( Journal.subject_id = ' + IntToStr(subjectId) + ' AND  Journal.date BETWEEN DateValue("' + firstDate + '") AND DateValue("' + endDate + '") ) ) AS j  RIGHT JOIN ( SELECT Users.id AS id, Users.surname as surname, Users.name AS name, Users.patronymic AS patronymic FROM Users INNER JOIN Groups ON Users.group_id = Groups.id WHERE Groups.id = ' + IntToStr(groupId) + ' ORDER BY Users.id ) AS u ON j.user_id = u.id) AS testing1,  ( SELECT dates.firs FROM ( SELECT Timetable.day_of_week FROM Timetable, Subjects, Groups WHERE ( Subjects.id = Timetable.subject_id AND Groups.id = Timetable.group_id AND  Subjects.id = ' + IntToStr(subjectId) + ' AND Groups.id = ' + IntToStr(groupId) + ' )) AS Timetable_get_group_subject, (' + selectDates + ') AS dates WHERE Timetable_get_group_subject.day_of_week = ( Weekday(dates.firs) - 1 ) ORDER BY dates.firs ) AS testing3 ) AS t LEFT JOIN Marks AS m ON t.mark_id = m.id GROUP BY t.fullname PIVOT DAY(t.firs)';
...
begin
  ...
  if (journalActionControllerCheckOnError(action, student, mark, date, errorLabel) = true) then
    Exit;
...
procedure TMain.buttonAddClick(Sender: TObject); begin journalActionController('add', subjectId, groupId, selectStudent, selectMark) end;
procedure TMain.buttonChangeClick(Sender: TObject); begin journalActionController('update', subjectId, groupId, selectStudent, selectMark) end;
procedure TMain.buttonDeleteClick(Sender: TObject); begin journalActionController('delete', subjectId, groupId, selectStudent, selectMark) end;
...
begin
  ...
  If selectGroup.Focused then 
    If (WheelDelta < 0) then bein
...
procedure TSubjects.buttonAddClick(Sender: TObject); begin subjectsActionController('add', nameEdit, audienceEdit, teacherComboBox, errorLabel, recordId); end;
procedure TSubjects.buttonChangeClick(Sender: TObject); begin subjectsActionController('update', nameEdit, audienceEdit, teacherComboBox, errorLabel, recordId); end;
procedure TSubjects.buttonDeleteClick(Sender: TObject); begin subjectsActionController('delete', nameEdit, audienceEdit, teacherComboBox, errorLabel, recordId); end;
...
procedure TTimetable.selectGroupClick(Sender: TObject); begin groupName := selectGroup.KeyValue; showTable(currentType); end;
procedure TTimetable.selectSubjectClick(Sender: TObject); begin subjectName := selectSubject.KeyValue; showTable(currentType); end;
end.
...
procedure TGroups.buttonAddClick(Sender: TObject); begin groupsActionController('add', nameEdit, groupId, errorLabel); end;
procedure TGroups.buttonChangeClick(Sender: TObject); begin groupsActionController('update', nameEdit, groupId, errorLabel); end;
procedure TGroups.buttonDeleteClick(Sender: TObject); begin groupsActionController('delete', nameEdit, groupId, errorLabel); end;
...
begin
  if ((String(id) = '') AND (action <> 'add')) then begin showErrorLabel('Ошибка ИД'); usersActionControllerCheckOnError := True; Exit; end;
  if action = 'delete' then begin usersActionControllerCheckOnError := False; Exit; end;
  if String(groupId) = '' then begin showErrorLabel('Ошибка ИД группы');  usersActionControllerCheckOnError := True; Exit; end;
  ...
  if patronymic = '' then begin showErrorLabel('Ошибка отчества'); usersActionControllerCheckOnError := True; Exit; end;
  if email = '' then begin showErrorLabel('Ошибка email'); usersActionControllerCheckOnError := True; Exit; end;
  if (username = '') AND (loginAndPassword = true)  then begin showErrorLabel('Ошибка username'); usersActionControllerCheckOnError := True; Exit; end;
  if (password = '') AND (loginAndPassword = true) then begin showErrorLabel('Ошибка password'); usersActionControllerCheckOnError := True; Exit; end;
